...
const CODEX_CLI_OUTPUT_LIMIT = 200000;

function looksLikeCodexCliModel(model) {
  const value = String(model || '').trim();
  if (!value) return false;
  if (value.startsWith('openai-codex/')) return true;
  if (value.startsWith('openai/')) return true;
  return /^(gpt-|o[134]|codex)/i.test(value);
}

export function resolveCodexCliModel(model) {
  const value = String(model || '').trim();
  if (!value) return null;
  if (!looksLikeCodexCliModel(value)) return null;
...
export function buildCodexExecInvocation({
  ...
  args.push('-');
  return { command: 'codex', args, resolvedModel };
}

function appendWithLimit(current, chunk, limit = CODEX_CLI_OUTPUT_LIMIT) {
  if (current.length >= limit) return current;
  const remaining = limit - current.length;
  return current + (chunk.length > remaining ? chunk.slice(0, remaining) : chunk);
}

function emitStreamLines(bufferState, chunk, stream, onEvent) {
  if (!onEvent) return;
  bufferState[stream] = (bufferState[stream] || '') + chunk;
  const lines = bufferState[stream].split(/\r?\n/);
  bufferState[stream] = lines.pop() || '';
  for (const line of lines) {
    const content = line.trimEnd();
    if (!content) continue;
    onEvent({ type: 'runtime-log', stream, content });
  }
}
...
export async function runAgentWithCodexCLI(opts) {
  const {
    prompt,
    model = null,
    cwd,
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hpc-agent-codex-cli-'));
  ...
  return await new Promise((resolve) => {
    ...
    timeoutHandle = timeoutMs > 0 ? setTimeout(() => {
      timedOut = true;
      log(`⏰ Codex CLI timeout after ${Math.floor((Date.now() - startTime) / 60000)}m`);
      killChild('SIGTERM');
    ...
    if (abortSignal) {
      if (abortSignal.aborted) {
        ...
        finish({
          ...
          durationMs: Date.now() - startTime,
          timedOut: false,
          rawOutput: 'Codex CLI run aborted before start.',
        });
        return;
      }
    ...
    log(`Codex CLI exec${invocation.resolvedModel ? ` (model: ${invocation.resolvedModel})` : ''}`);
    ...
    child.on('error', (error) => {
      finish({
        ...
        durationMs: Date.now() - startTime,
        timedOut: false,
        rawOutput: `Codex CLI failed to start: ${error.message}`,
      });
    });
