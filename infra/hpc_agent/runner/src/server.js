...
class ProjectRunner {
  ...
  _buildSingleManagerContext(config, orchestration) {
    ...
    const lines = [
      ...
      '',
      '> Use one manager cycle to plan and dispatch workers. Prefer enough independent READY GPU experiment tasks to keep available GPUs busy.',
      '> When dependency structure matters, prefer TASK_GRAPH with id/depends_on/depends_on_tags/produces_tags/replan_after.',
      '> In TASK_GRAPH, prefer worker_class for pooled workers, for example {"worker_class":"experiment_runner"}, and use agent only when a specific worker is required.',
      '> If later work depends on experiment outputs, use fan-out/fan-in: many independent experiments, then one analyst step.',
      '> If a worker needs shared-allocation compute, include a resources object, for example {"gpus": 1, "cpus": 1}.',
      '> For autoresearch, treat completed experiment waves as feedback for the next wave; do not stop after one successful wave unless the project explicitly says one wave is enough.',
      ...
      '> If the project specifies a minimum iteration or wave count, do not emit PROJECT_COMPLETE before that count is reached.',
      ...
      '> For autoresearch training on this node, workers can use infra/hpc_agent/runner/scripts/run-autoresearch-worker.sh after you grant GPUs.',
      '> If the project is actually complete, emit <!-- PROJECT_COMPLETE --> with success=true.',
    ];
    return lines.join('\n');
  }
